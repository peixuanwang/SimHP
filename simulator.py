    ...
    while True:
        ...
        if tmp is not None:
            ...
            if dise not in dis_name:
                dis_name[dise] = name
            freq = 1
            info = re.match('(\d+)/(\d+)',it[14])
            if info is not None:
                freq = float(info.group(1))/float(info.group(2))
            info = re.match('(\d+)\sof\s(\d+)',it[14])
            if info is not None:
                freq = float(info.group(1))/float(info.group(2))
            info = re.match('(\d+)%',it[14])
            if info is not None:
            ...
            if info is not None:
                freq = (int(info.group(1))+int(info.group(2)))*0.01/2
            info = re.match('[Cc]ommon',it[14])
            if info is not None:
                freq = 0.5
            info = re.match('[Ff]requent',it[14])
            if info is not None:
                freq = 0.33
            info = re.match('[Hh]allmark',it[14])
            if info is not None:
                freq = 0.9
            info = re.match('[Oo]bligate',it[14])
            if info is not None:
                freq = 1
            info = re.match('[Oo]ccasional',it[14])
            if info is not None:
                freq = 0.1
            info = re.match('[Rr]are',it[14])
            if info is not None:
                freq = 0.05
            info = re.match('[Tt]ypical',it[14])
            if info is not None:
                freq = 0.5
            info = re.match('[Vv]ariable',it[14])
            if info is not None:
                freq = 1
            info = re.match('[Vv]ery\srare',it[14])
            if info is not None:
                freq = 0.01
            info = re.match('[Vv]ery\sfrequent',it[14])
            if info is not None:
                freq = 0.45
            hp = it[4].strip()
...
def sim(_omim,obo,length,impre,noi,disease_hpo,dis_hp_freq,hpall,o,freq):    
    fhp = []
    orghp = []
    prthp = []
    unrelatedhp = []
...
def usage():
    ...
    print( 'table is 2018-03-08. You can download the newest files from HPO website:')
    print( 'http://human-phenotype-ontology.github.io/downloads.html ')
    ...
    print( '         Set this parameter if you want to use frequency.\n\n')
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hafo:b:t:c:i:l:m:n:r:")
    ...
    for omimid in omimlists:
        f.write("\n\n" + omimid + "\t" + dis_name[omimid] + "\n\n" )
        for a in range(int(case)):
            patienthp = sim(omimid,obo,length,impre,noi,disease_hpo,dis_hp_freq,hpall,o,freq)
            if not anno:
                s='\t'.join(patienthp)
                f.write(omimid + "\t" + s + "\n" )
            else:
